...
This section describes the end-to-end flow of using the Workplace Zakat Platform. The application separates **HR administration** (enrollment, payroll, and blockchain finalization) from **employee verification** (read-only receipts). Only the HR Admin interacts with MetaMask; employees use a standard web form.
...
flowchart TD
    A[Home /] --> B{User role}
    B -->|HR Admin| C[/hr HR Dashboard]
    B -->|Employee| D[/employee Receipt portal]

    C --> E[Create or update enrollment]
    E --> F[Draft payroll period]
    F --> G[Sync Team]
    G --> H[Finalize Period]
    H --> I[On-chain append via MetaMask]
    I --> J[Local audit chain + DB status FINALIZED]

    D --> K[Lookup by work email]
    K --> L[Display finalized deductions only]
```

### 2.2 HR Admin process

1. **Entry** — From the home page, the administrator opens **Access HR Console** (`/hr`).
2. **Enrollment** — Under **New Enrollment**, select an employee, set a fixed **monthly deduction** amount, and submit. The system deactivates any prior enrollment for that employee, creates a new active plan, and appends an `ENROLLMENT_CREATED` event to the local SHA-256 audit chain.
3. **Draft payroll** — Under **Payroll Periods**, enter a period label (e.g. `May 2026`) and click **Draft New Period**. A payroll run is created with status `DRAFT` and logged as `PAYROLL_RUN_CREATED`.
4. **Generate deductions** — On the draft period card, click **Sync Team**. The server reads all **active** enrollments and creates or updates one deduction line per employee (`DEDUCTIONS_GENERATED` audit event).
5. **Finalize** — Click **Finalize Period**. The client requires MetaMask on Sepolia: it builds a JSON payload for the payroll run, calls `append()` on `ZakatAuditLog.sol`, and only after a successful transaction invokes the server action to set the run to `FINALIZED`, stamp `finalizedAt`, and append `PAYROLL_RUN_FINALIZED` to the audit chain.
6. **Audit review** — The **System Activity Log** at the bottom of the HR page lists hash-linked blocks (`prev_sig` → `data_sig`) so administrators can confirm the integrity of all recorded actions.

Enrollments can be **paused** or **resumed** at any time; only active enrollments are included when syncing a draft period.

### 2.3 Employee process

1. **Entry** — From the home page (**View Sample Receipt**) or directly at `/employee`.
2. **Lookup** — Enter a corporate work email and submit. No wallet or cryptocurrency knowledge is required.
3. **Receipt** — If the employee exists, the portal lists each deduction tied to a **finalized** payroll period: period label, amount, and verification date. A running **total contribution** is shown. Draft or unfinalized periods never appear, so employees only see employer-confirmed records.

Demo accounts (after seeding the database) include `sara.elamrani@demo.local`, `youssef.haddad@demo.local`, and `layla.mansour@demo.local`.

### 2.4 Web2 vs Web3 boundary

| Stage | Layer | Data stored |
|-------|--------|-------------|
| Enrollment, draft, sync | Web2 (PostgreSQL + local hash chain) | Names, emails, amounts, enrollment state |
| Finalize Period | Web3 (Sepolia + smart contract) | Cryptographic hash of finalize payload; HR wallet as registrar |
| Employee receipt | Web2 read-only | Finalized deduction history derived from PostgreSQL |

Physical movement of fiat funds is out of scope; finalization is an **auditing and notarization** step, not a token transfer.
